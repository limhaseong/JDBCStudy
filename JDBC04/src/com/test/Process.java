...
public class Process
{
	...
	public void sungjukInsert()
	{
		...
		// 반복문 구성 -- (4)
			...
			// 입력값 담아내기(국어, 영어, 수학) -- (7)
			// ScoreDTO 객체 생성 -- (9)
			// 입력값을 활용하여 ScoreDTO 객체 구성 -- (10)
			// 데이터베이스 입력 액션 처리(add()) <- ScoreDTO 필요 -- (8)
			// 입력 처리 정상 여부 확인 -- (11)
		// 반복문 끝 -- (12)
		// 데이터베이스 연결 종료 -- (13)
		try
	...
	public void sungjukSelectAll()
	{
		try
		{
			...
			// dao 의 close() 메소드 호출 -> 데이터 베이스 연결 종료
			dao.close();
	...
	public void sungjukUpdate()
	{
		try
		{
			...
			if (arrayList.size() > 0)
			{
				...
				dto.setEng(eng);
				dto.setMat(mat);
				dto.setSid(String.valueOf(sid)); // check~!! --> 위에 int로 선언했는데 ScoreDTO에서 String으로 선언해놓은 거 확인.
